...
<?php
// insert navigation to plugins
if ($this->Filter == 'all') {
   $HtmlOut = '<div class="Info" id="PluginNavigator">Index: ';
   $LastLetter = '';
   foreach ($this->AvailablePlugins as $PluginName => $PluginInfo) {
      $FirstLetter = strtoupper(substr(GetValue('Name', $PluginInfo, $PluginName), 0, 1));
      if ($FirstLetter != $LastLetter) {
         $PluginLink = '#'.Gdn_Format::Url(strtolower($PluginName)).'-plugin"';
         $HtmlOut .= '&nbsp;&nbsp;<a href="'.$PluginLink.'">'.$FirstLetter.'</a>';
      }
...
foreach ($this->AvailablePlugins as $PluginName => $PluginInfo) {
   ...
   if ($this->Filter == 'all' || $this->Filter == $State) {
      ...
      <tr <?php echo 'id="'.Gdn_Format::Url(strtolower($PluginName)).'-plugin"', ' class="More '.$RowClass.'"'; ?>>
         ...
         <th><?php echo $ScreenName; ?>
<a href="#<?= Gdn_Format::Url(strtolower($PluginName)) ?>-plugin" accesskey="<?= strtolower(substr($PluginName, 0, 1)) ?>"></a>
<a href="#PluginNavigator" style="float:right">&#x25B2;</a></th>
